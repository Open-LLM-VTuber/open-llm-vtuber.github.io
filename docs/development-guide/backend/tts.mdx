...
3. 在 `conf.yaml` 以及 `conf.CN.yaml` 文件中，添加 tts 配置项
...
### 2.3 在 `conf.yaml` 以及 `conf.CN.yaml` 文件中，添加 tts 配置项

前往 `conf.yaml` 与 `conf.CN.yaml` (conf.yaml 文件的中文版预设)，往 `tts_config` 中添加新 tts 需要的配置项。
